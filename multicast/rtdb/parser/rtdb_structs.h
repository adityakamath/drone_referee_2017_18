...
/* Structure to store a network */
typedef struct
{
	unsigned num;				// Unique network identification number (0-n)
	char* id;					// network name (alfanum)
	rtdb_ItemList sharedItems;	// Shared items list in schema
} rtdb_Network;
...
typedef struct
{
	rtdb_Network* networks;		// Dynamic network list
	unsigned numNw; 			// Total number of networks in list
} rtdb_NetworkList;
