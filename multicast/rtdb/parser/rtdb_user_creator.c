...
*/
int printUserFile(rtdb_ItemList* it, rtdb_AgentList* ag, rtdb_NetworkList* nw)
{
	unsigned i;
	FILE *f;
	char* command;
...
int printNetworkFile(rtdb_NetworkList* nw)
{
	...
	//If for some reason we can't create the file, abort
	if ((f= fopen(RTDB_NETWORK_H, "w")) == NULL)
	{
		return 2;
	}

	//Write generic information to the file
	fprintf(f, "/* AUTOGEN FILE : rtdb_network.h */\n\n");
	fprintf(f, "#ifndef _RTDB_NETWORK_\n#define _RTDB_USER_\n\n");

	//Write a generic comment
	fprintf(f, "/* network section */\n\n");

	//Run through the item list and write all the defines
	for(i= 0; i < nw->numNw; i++)
	{
		fprintf(f, "#define %s\t%d\n", nw->networks[i].id, nw->networks[i].num);
	}

	if(nw->numNw > 0)
	{
		fprintf(f, "\n/* network names */\n\n");
		fprintf(f, "const char const * network_names[]={");
		for(i= 0; i < nw->numNw; i++)
		{
			if(i>0)
			{
