...
{
	if(addSharedItem(&sc->sharedItems, itemId )){
...
//Add an item to a shared items list, returns 1 is new item is added, 0 if item is already present in the list
int addSharedItem(rtdb_ItemList* psharedItemsList, char* itemId )
{
	unsigned i, itemIndex, itemExists;
	//Check if the items list isn't empty
	if (itList.numIt == 0)
	{
		char * err= malloc((1+strlen("Tentativa de adicionar o item \e[32m")+strlen(itemId)+strlen("\e[0m inexistente \e[33m(A lista de items está vazia)\e[0m!")) * sizeof(char));
		sprintf(err, "Tentativa de adicionar o item \e[32m%s\e[0m inexistente \e[33m(A lista de items está vazia)\e[0m!", itemId);
		abortOnError(err);
	}
	else
	{
	...
	if (!itemExists) {
		char * err= malloc((1+strlen("\nTentativa de adicionar o item \e[32m")+strlen(itemId)+strlen("\e[0m \e[33minexistente na lista de items!")) * sizeof(char));
		sprintf(err, "\nTentativa de adicionar o item \e[32m%s\e[0m \e[33minexistente na lista de items!", itemId);
		abortOnError(err);
	}
	//If the shared items list is empty initialize it with the item
	if (psharedItemsList->numIt == 0)
	{
		//If in Debug mode inform the user about the list initialization
		if (DEBUG)
		{
			printf("\nA inicializar a lista de items partilhados com o item \e[32m%s\e[0m na posicao \e[32m%u\e[0m\n", itList.items[itemIndex].id, psharedItemsList->numIt);
		}
		psharedItemsList->items[psharedItemsList->numIt++] = itList.items[itemIndex];
	}
	else
	{
		//If the list isn't empty run the items list
		for(i = 0; i < psharedItemsList->numIt; i++)
		{
			//If the item already exists in the list ignore the addition
			if ( strcmp(psharedItemsList->items[i].id, itemId) == 0 )
			{
				//If in Debug mode inform the user about the item existance
				if (DEBUG)
				{
					printf("\nO item \e[32m%s\e[0m já existe na lista de items partilhados na posicao \e[32m%u\e[0m. A ignorar re-declaração!\n", itemId, i);
				}
				return 0;
			}	//If the item doesn't already exist in the shared items list of the schema add it
			else if (i == psharedItemsList->numIt-1)
			{
				//If in Debug mode inform the user that the item was correctly added
				if (DEBUG)
				{
					printf("\nA adicionar item \e[32m%s\e[0m na posicao \e[32m%u\e[0m da lista de \e[32m%u\e[0m items partilhados\n", itList.items[itemIndex].id, psharedItemsList->numIt, psharedItemsList->numIt+1);
				}
				psharedItemsList->items[psharedItemsList->numIt++] = itList.items[itemIndex];
				break;
			}
...
/**************  Network manipulation Function  **************/

//Add a network to the networks global list if it wasn't previouly declared
rtdb_Network* networkCreate(char* newId)
{
	unsigned i;
	//Run through the network list
	for(i = 0; i < networkList.numNw; i++)
	{
		//If the schema was previouly defined return a pointer to it
		if (strcmp(networkList.networks[i].id, newId) == 0)
		{
			//If in Debug mode tell the user
			if (DEBUG)
			{
				printf("\nAlready found network \e[32m%s\e[0m at position \e[32m%u\e[0m out of \e[32m%u\e[0m networks.\n", newId, i, networkList.numNw);
			}
			return &networkList.networks[i];
		}
	}
	//Add the schema to the schemas list
	networkList.networks[networkList.numNw].id = strdup(newId);
	networkList.networks[networkList.numNw].num = networkList.numNw;
	//Initialize the shared items list
	networkList.networks[networkList.numNw].sharedItems.numIt = 0;
	networkList.networks[networkList.numNw].sharedItems.items = malloc(sizeof(rtdb_Item));
	//Increment the network list counter
	networkList.numNw++;
	//Reallocate memory for the schemas list
	networkList.networks = realloc(networkList.networks, (networkList.numNw+1)*sizeof(rtdb_Network) );

	//If in Debug mode tell the user the network was added to the list
	if (DEBUG)
	{
		printf("\nNetwork \e[32m%s\e[0m successfully added to the list of networks at position \e[32m%u\e[0m!\n", newId, networkList.numNw-1);
	}

	//Return a pointer to the network
	return &networkList.networks[networkList.numNw-1];
}

//Add an item to the shared items list in the network specified if it wasn't previouly defined
void networkAddSharedItem(rtdb_Network* nw, char* itemId)
{
	if(addSharedItem(&nw->sharedItems, itemId )){
		//Reallocate memory for the shared items list
		nw->sharedItems.items = realloc(nw->sharedItems.items, (nw->sharedItems.numIt+1)*sizeof(rtdb_Item));
	}
}

// free the network list
void freeNetworkList()
{
	unsigned i;
	//Run through the network list
	for(i = 0; i < networkList.numNw; i++)
	{
		free(networkList.networks[i].sharedItems.items);
	}

	free(networkList.networks);
}
