...
*@File: xrtdb.y                                                 *
*@Description: Syntatic analizer and automatic configuration    *
*                 files generator                               *
*****************************************************************/

%pure_parser
%start S
%token agentsDECL networkDECL itemDECL schemaDECL assignmentDECL                                            /* Config file items keywords */
...
INITIAL:    eol { nline++; } INITIAL
          ...
          | networkDECL identifier { pNetwork= networkCreate($2); } NETWORKOPEN INITIAL
...
NETWORKOPEN:   eol { nline++; } NETWORKOPEN
            | openbrace NETWORK
            | error { raiseError(nline, _ERR_NETWORKOPEN_); }
            ;

NETWORK:     eol { nline++; } NETWORK
          | sharedFIELD equal NETWORKITEMS semicomma NETWORK
          | sharedFIELD equal NETWORKITEMS NETWORK
          | closebrace
          | error { raiseError(nline, _ERR_NETWORKFIELD_); }
          ;

NETWORKITEMS:      identifier { networkAddSharedItem(pNetwork, $1); }
                	| NETWORKITEMS comma opt_eol identifier { networkAddSharedItem(pNetwork, $4); }
                	| error { raiseError(nline, _ERR_ITEMSLIST_); }
                	;
